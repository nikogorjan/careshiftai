...
const MEMBERS = [
  { name: "Kris Ghimire", role: "Founder & CEO", src: ceo, position: "object-[50%_20%]" },
  { name: "Caroline Chatham", role: "Chief Communication Officer", src: cco, position: "object-[50%_20%]" },
  { name: "Gloria Sanchez", role: "Business Development Manager", src: bdm, position: "object-[50%_20%]" },
  /* Full-body seated source; the higher focal point keeps the crop on head and shoulders. */
  { name: "Adithya Royaprollu", role: "Sr. AI Engineer", src: aiEngineer, position: "object-[50%_8%]" },
];
    ...
    <section id="team" aria-labelledby="team-h" className="section-y bg-white">
      <div className="wrap">
        ...
        <ul className="section-gap m-0 grid list-none grid-cols-2 gap-4 p-0 sm:gap-6 lg:grid-cols-4">
          {MEMBERS.map((m, i) => (
            <Reveal as="li" key={m.name} delay={i * 80} className="m-0">
              ...
              <div className="mt-4">
                ...
                <p className="m-0 text-sm font-medium text-ink sm:text-[15px]">{m.name}</p>
                <p className="m-0 mt-1 text-sm text-ink-3">{m.role}</p>
              </div>
            </Reveal>
          ))}
        </ul>
